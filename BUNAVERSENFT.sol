...
interface ILuckyEggs is IERC721 {
    ...
    function FundWalletView() external;
...
contract Bunnies is ERC721 {
    ...
    // Pills own;
    uint private Pills_newTokenID = 88888;
    uint private Pills_StartIndex = Pills_newTokenID;
    uint public PillsQuantity = 1112;
    uint public PillsMinted = 0;
    ...
    // Owner addresses
    address payable fundWallet1;
    address payable fundWallet2;
    ...
    /* Recycle Bunnies */
    function Recycle(address player, uint token1_ID, uint token2_ID) public {
        require(RecycleActive == true, "Recycling is not active yet");
        require(token1_ID != token2_ID, "Same tokens cannot recycle :P ");
        require(token1_ID < Pills_StartIndex, "Please check token 1");
        require(token2_ID < Pills_StartIndex, "Please check token 2");
        ...
        _mint(player, Recycle_NextIndex);
        Recycle_NextIndex += 1;
        RecycleCount +=  1;
        Sacrifice(token1_ID);
        Sacrifice(token2_ID);
    ...
    function ChangePillsLimit(uint  add_newTokens) public{
        require(msg.sender == owner || msg.sender == fundWallet1, "Only owner");
        PillsQuantity += add_newTokens;
    }

    /* Love Pill */
    function MintPills(address[] memory _players) public {
        ...
        for (uint256 i = 0; i < numberOfPlayers; i++) {
            require(PillsQuantity != PillsMinted, "No Love Pill Available.");
            _mint(_players[i], Pills_newTokenID);
            PillsMinted++;
            Pills_newTokenID++;
        }
    }
    ...
    /* Breed Bunnies */
    function Breed(address player, uint token1_ID, uint token2_ID, uint Pills_tokenId) public payable {
        require(token1_ID != token2_ID, "You don't need to come here for self breeding");
        require(token1_ID < Pills_StartIndex, "You can only Breed Bunnies... Please check token 1!");
        require(token2_ID < Pills_StartIndex, "You can only Breed Bunnies... Please check token 2!");
        require(Pills_tokenId >= Pills_StartIndex, "No Love without Love Pill...");
        require(ownerOf(Pills_tokenId) == msg.sender, "Own Love Pill for Breeding!!!");
        ...
        _mint(player, Breed_NextIndex);
        Breed_NextIndex += 1;
        BreedCount +=  1;
        Sacrifice(Pills_tokenId);
    }
    ...
    /* CrossBreed Bunnies */
    function CrossBreed(address player, uint token1_ID, uint token2_ID, uint Pills_tokenId) public payable {
        ...
        _mint(player, CrossBreed_NextIndex);
   ...
   function ChangeIdentity(uint _tokenId, string memory _name, string memory _desc) public {
    ...
    function WithdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(fundWallet2).send(percent * 50)); // Arka
    }
